...
vignette: |
  %\VignetteIndexEntry{cuyem} %\VignetteEngine{knitr::rmarkdown} %\VignetteEncoding{UTF-8}
---
...
Three methods exist for estimating confidence intervals; 'wald', 'exact' and 'score'.  The DEFAUT method is 'score' which provides strictly positive confidence interval estimates with nominal coverage probabilities.  

The *'wald'* method has the potential to produce confidence intervals outside the range of [0,1]. 

```{r}
p_hat <- estimate_proportion(x = 1, n = 10, method = 'wald')
```

```{r}
p_hat <- estimate_proportion(x = 9, n = 10, method = 'wald')
```

And the *'exact'* method tends to produce too wide of confidence intervals that exceed the desired coverage probabilites.

```{r}
p_hat <- estimate_proportion(x = 5, n = 10, method = 'exact')
```

```{r}
exact_width <- p_hat$CI.upper - p_hat$CI.lower
exact_width
```

```{r}
p_hat <- estimate_proportion(x = 5, n = 10, method = 'score')
```
